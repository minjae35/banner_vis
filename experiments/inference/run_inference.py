...
"""
"""
- `config.py`, `inference_utils.py`에 정의된 헬퍼를 불러서 씀
- 결과는 infernece_results에 저장
"""
...
from inference_utils import (                     # inference_utils.py
...
def run_inference_for_checkpoint(checkpoint_name, images, prompt_type="standard", max_images=None):
    ...
    # 체크포인트 설정 가져오기
    config = get_checkpoint_config(checkpoint_name)            # 8개의 checkpoint (bal_equal, bal_equal_simple, c3f2w1, c3f2w1_simple etc)
    checkpoint_path = config["path"]
    gpu_id = config["gpu_id"]                                  # gpu designation
    ...
    try:
        for image_id, image_path in tqdm(images, desc=f"{checkpoint_name} 추론"):
            result = run_single_inference(model, processor, device, image_path, prompt_text)          # includes ocr results
...
def run_all_checkpoints_inference(data_file, prompt_type="standard", max_images=None, checkpoint_names=None):
    ...
    for i, checkpoint_name in enumerate(checkpoint_names):
        try:
            ...
            results = run_inference_for_checkpoint(                         # includes ocr results
                checkpoint_name, 
                images, 
                prompt_type, 
                max_images
...
def main():
    ...
    if args.single_checkpoint:
        ...
        # 이미지 로드
        images = load_images_from_jsonl(args.data_file, args.max_images)        # # warp, flat, crop 각각 8장씩 (총 24장): data/experiments/validation_test
        if not images:
            print("[ERROR] 로드할 이미지가 없습니다.")
            return
        
        # 단일 체크포인트 추론 (한 체크포인트에 대해 전 과정 실행)
