...
"""
This contains a Detection model
"""
...
def get_args():
    ...
    args.image = '/home/intern2/banner_vis/src/sample_img'
...
def process_image(img_path, args, device, det_model, test_pipeline, processor, model):
    ...
    for i, r in enumerate(results):
        ...
        annotation.append({                                                                         # 감지된 박스 좌표, 신뢰도, 크롭 이미지 경로
            "bbox_normalized": r["bbox_normalized"],
            "bbox_abs":         r["bbox_abs"],
            "confidence":       r["confidence"],
            "crop_path":        r["crop_path"]
        })
        summary[str(i)] = {"ori_txt": ori_txt, "ban_cls": ban_cls, "reason": reason}                # 사람이 보기 좋게 Qwen 요약 결과
    out = {                                                                                         # 최종 JSON 구조로 묶기
        "image_name":       img_path,
        "width":            width,
        "height":           height,
    ...
    with open(out_path, "w", encoding="utf-8") as f:                                                # 파일 저장
