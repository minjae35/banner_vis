...
def banWarp(seg_out, imgs, idx, args):
    '''
    세그멘테이션(pred_mask)를 기반으로
    현수막(또는 직사각형 영역)의 네 모서리를 찾고,
    "원근 변환(perspective transform)"으로 이미지를 "정면에서 본" 형태로 보정(warping)하는 함수.
    '''
    warp_out = []
    ...
    for k, pred_mask in enumerate(seg_out):
        ...
        # Step 1: 원본 이미지와 세그멘테이션 마스크 시각화
        if args.warping.save_results:
            os.makedirs(os.path.join(args.save_root_path, "warp_steps"), exist_ok=True)
            
            # 원본 이미지 저장
            orig_filename = args.out_img_filename.split(".")
            orig_filename[0] += f"_orig_{idx[k][0]}"
            orig_filename = ".".join(orig_filename)
            cv2.imwrite(os.path.join(args.save_root_path, "warp_steps", orig_filename),
                       cv2.cvtColor(img.astype(np.uint8), cv2.COLOR_RGB2BGR))
            
            # 세그멘테이션 마스크 시각화
            mask_vis = np.zeros_like(img)
            mask_vis[pred_mask > 0] = [0, 255, 0]  # 녹색으로 마스크 표시
            mask_filename = args.out_img_filename.split(".")
            mask_filename[0] += f"_mask_{idx[k][0]}"
            mask_filename = ".".join(mask_filename)
            cv2.imwrite(os.path.join(args.save_root_path, "warp_steps", mask_filename),
                       cv2.cvtColor(mask_vis.astype(np.uint8), cv2.COLOR_RGB2BGR))
        ...
        # Step 2: 찾은 모서리 시각화
        if args.warping.save_results:
            corners_vis = img.copy().astype(np.uint8)
            for i, corner in enumerate(four_corners):
                cv2.circle(corners_vis, tuple(corner), 10, (255, 0, 0), -1)  # 파란색 원
                cv2.putText(corners_vis, str(i), tuple(corner), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
            
            # 모서리들을 선으로 연결
            for i in range(4):
                pt1 = tuple(four_corners[i])
                pt2 = tuple(four_corners[(i + 1) % 4])
                cv2.line(corners_vis, pt1, pt2, (0, 255, 255), 3)  # 노란색 선
            
            corners_filename = args.out_img_filename.split(".")
            corners_filename[0] += f"_corners_{idx[k][0]}"
            corners_filename = ".".join(corners_filename)
            cv2.imwrite(os.path.join(args.save_root_path, "warp_steps", corners_filename),
                       cv2.cvtColor(corners_vis.astype(np.uint8), cv2.COLOR_RGB2BGR))
        ...
        # Step 3: 변환 행렬 정보 저장
        if args.warping.save_results:
            transform_info = f"Output size: {output_size}\n"
            transform_info += f"Transform matrix:\n{transform_matrix}\n"
            transform_info += f"Banner type: {'Horizontal' if is_long_banner_horizontal(four_corners) else 'Vertical'}"
            
            info_filename = args.out_img_filename.split(".")
            info_filename[0] += f"_transform_info_{idx[k][0]}.txt"
            info_filename = ".".join(info_filename)
            with open(os.path.join(args.save_root_path, "warp_steps", info_filename), 'w') as f:
                f.write(transform_info)
        ...
        cv2_img = cv2.cvtColor(img.astype(np.uint8), cv2.COLOR_BGR2RGB)

        output_image = cv2.warpPerspective(cv2_img, transform_matrix, output_size)

        output_image = cv2.cvtColor(output_image.astype(np.uint8), cv2.COLOR_BGR2RGB)

        # Step 4: 최종 warped 이미지 저장
        if args.warping.save_results:
            warp_filename = args.out_img_filename.split(".")
            warp_filename[0] += f"_warped_{idx[k][0]}"
            warp_filename = ".".join(warp_filename)
            cv2.imwrite(os.path.join(args.save_root_path, "warp_steps", warp_filename),
                       cv2.cvtColor(output_image.astype(np.uint8), cv2.COLOR_RGB2BGR))
            
            # Step 5: 전체 과정을 하나의 이미지로 합성
            # 원본, 마스크, 모서리, 최종 결과를 가로로 배치
            h, w = img.shape[:2]
            composite = np.zeros((h, w * 4, 3), dtype=np.uint8)
            
            # 원본 이미지
            composite[:, :w] = img.astype(np.uint8)
            
            # 마스크 오버레이
            mask_overlay = img.copy().astype(np.uint8)
            mask_overlay[pred_mask > 0] = mask_overlay[pred_mask > 0] * 0.7 + np.array([0, 255, 0]) * 0.3
            composite[:, w:w*2] = mask_overlay
            
            # 모서리 표시
            composite[:, w*2:w*3] = corners_vis
            
            # 최종 결과 (크기 조정)
            warped_resized = cv2.resize(output_image, (w, h))
            composite[:, w*3:] = warped_resized
            
            composite_filename = args.out_img_filename.split(".")
            composite_filename[0] += f"_composite_{idx[k][0]}"
            composite_filename = ".".join(composite_filename)
            cv2.imwrite(os.path.join(args.save_root_path, "warp_steps", composite_filename),
        ...
        if args.warping.save_results:
            os.makedirs(os.path.join(args.save_root_path), exist_ok=True)
...
def banDet(det_model, test_pipeline, imgs_array, imgs, params, data):
    """
    입력:
        - det_model: 사전에 학습된 현수막 탐지 모델 (ex.YOLO, RTMDet, GroundingDINO 등)
        - img_array, imgs: 전처리된 이미지 배열 (torch tensor, numpy array)
        - params : 탐지 설정값 (confidence threshold, 저장 옵션 등)
        - data : 각 프레임/이미지의 메타데이터 (폭, 높이, 시간 등)
    출력:
        - 탐지된 현수막의 바운딩 박스 좌표 + confidence score + 라벨
        - (형태: [x1, y1, x2, y2, conf, label])
    """
...
class Banner:
    ...
    def inference(self, cctv_id, path, models, params):
        (self.det_model, self.test_pipeline), self.std_model, self.str_model, self.ctr_model, self.ref_cls_model, self.seg_model = models
        ...
        # 디버깅을 위한 간단한 로그 추가
        print(f"Debug: len(std_results)={len(std_results)}, len(poly_results)={len(poly_results)}")
        print(f"Debug: len(four_corners)={len(four_corners)}, len(warped_idxes)={len(warped_idxes)}")
        print(f"Debug: len(indices)={len(indices)}")
        ...
        for k, (std_result, poly_result) in enumerate(zip(std_results, poly_results)):
            ...
            for i, (text, poly, prob) in enumerate(zip(res, poly_result, probs)):
                ...
                # indices[k] 안전성 체크
                if k < len(indices) and indices[k] < len(data) and indices[k] < len(banner_num_str):
                    current_banner_num = banner_num_str[indices[k]].item()
                    ...
                    # banner_annotation 인덱스 체크
                    if current_banner_num < len(data[indices[k]]["banner_annotation"]):
                        annotation["instance_id"] = data[indices[k]]["banner_annotation"][current_banner_num]["num_instances"]
                        annotation["banner_id"] = current_banner_num
                        data[indices[k]]["banner_annotation"][current_banner_num]["num_instances"] += 1
                        annotation["score_cls"] = prob.prod().item()
                        data[indices[k]]["annotation"].append(annotation)

                # banner_num_str 증가도 안전하게
                if k < len(indices) and indices[k] < len(banner_num_str):
                    banner_num_str[indices[k]] += 1
...
class config:
    params = {
        ...
        "font_path": "/home/intern/banner_vis/src/malgun.ttf",
        "sample_rate": 1,
        "camid": 0,
        "verbose": False,
        "preprocess": {
...
def banner_init_gpu(gpu_id, config_path):
    ...
    return args, (det_model, test_pipeline), std_model, str_model, ctr_model, ref_cls_model, sam_model

def banner_analysis(cctv_id, in_img_filename, out_img_filename, out_metadata, models, gpu_id, enable_warp_visualization=False):
    ...
    # Warping 시각화 활성화
    if enable_warp_visualization:
        params.warping.save_results = True
...
if __name__ == '__main__':
    cctv_id = 2
    gpu_id = 7
    conf_path = "/home/intern/banner_vis/configs/tmpv5.json"
